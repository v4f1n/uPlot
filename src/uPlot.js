...
import { fmtDate, tzDate } from "./fmtDate";
...
function snapLogY(self, dataMin, dataMax, scale) {
	return dataMin == null
		? nullNullTuple
		: rangeLog(dataMin, dataMax, self.scales[scale].log, true);
}
...
function snapAsinhY(self, dataMin, dataMax, scale) {
	return dataMin == null
		? nullNullTuple
		: rangeAsinh(dataMin, dataMax, self.scales[scale].log, true);
}
...
export default function uPlot(opts, data, then) {
	...
	on(
		...
		(e) => {
			if (e.target === over) {
				let didDrag =
					mouseLeft1 != mouseLeft0 || mouseTop1 != mouseTop0;
			...
			sc.distr == 3
				? (val) =>
						log10(
							val > 0
								? val
								: sc.clamp(self, val, sc.min, sc.max, sc.key)
						)
				: sc.distr == 4
				? (val) => asinh(val, sc.asinh)
				: sc.distr == 100
				? (val) => sc.fwd(val)
				: (val) => val;
	...
	const _tzDate =
		FEAT_TIME && (opts.tzDate || ((ts) => new Date(round(ts / ms))));
	...
	const _timeAxisVals =
		...
		timeAxisVals(
			_tzDate,
			timeAxisStamps(
				ms == 1 ? _timeAxisStampsMs : _timeAxisStampsS,
				_fmtDate
			)
		);
	const _timeSeriesVal =
		FEAT_TIME &&
		timeSeriesVal(_tzDate, timeSeriesStamp(_timeSeriesStamp, _fmtDate));

	const activeIdxs = [];

	const legend =
		FEAT_LEGEND && (self.legend = assign({}, legendOpts, opts.legend));
	...
	function initLegendRow(s, i) {
		...
		if (s.label instanceof HTMLElement) text.appendChild(s.label);
		else text.textContent = s.label;
	...
	if (cursor.dataIdx == null) {
		...
		cursor.dataIdx = (self, seriesIdx, cursorIdx, valAtPosX) => {
			...
			if (skip.has(yValues[cursorIdx])) {
					...
					if (withProx) {
						...
						idx2 =
							nonNullRgt == null
								...
								: nonNullLft == null
								? nonNullRgt
								: cursorIdx - nonNullLft <=
								  nonNullRgt - cursorIdx
								? nonNullLft
								: nonNullRgt;
					}
				}
				...
				let dist = abs(
					cursorLft - valToPosX(xValues[cursorIdx], scaleX, xDim, 0)
				);
	...
	function addSeries(opts, si) {
		...
		opts =
			mode == 1
				...
				: setDefault(opts, si, {}, xySeriesOpts);
	...
	function accScale(wsc, psc, facet, data, sorted = 0) {
		if (
			...
			let minMax =
				facet.min == null
					? getMinMax(data, _i0, _i1, sorted, wsc.distr == 3)
					: [facet.min, facet.max];
	...
	function setScales() {
		...
		for (let k in pendScales) {
			...
			if (psc != null) {
				...
				if (psc.min != null) assign(wsc, psc);
	...
	function drawSeries() {
		if (dataLen > 0) {
			let shouldAlpha =
				series.some((s) => s._focus) && ctxAlpha != focus.alpha;
			...
			series.forEach((s, i) => {
				if (i > 0 && s.show) {
					...
					if (FEAT_POINTS) {
						...
						if (show || idxs) {
							s.points._paths = s.points.paths(
								self,
								i,
								i0,
								i1,
	...
	function axesCalc(cycleNum) {
		...
		axes.forEach((axis, i) => {
			...
			let splits =
				scale.distr == 2 ? _splits.map((i) => data0[i]) : _splits;
			let incr =
				scale.distr == 2
	...
	function drawAxesGrid() {
		for (let i = 0; i < axes.length; i++) {
			...
			if (axis.label != null) {
				...
				let _label = isFn(axis.label)
					? axis.label(self, i, _incr, _space)
					: axis.label;
	...
	function toggleDOM(i) {
		...
		if (s.show) showLegend && remClass(legendRows[i], OFF);
		else {
	...
	function updateCursor(src, _fire, _pub) {
		...
		if (mouseLeft1 < 0 || dataLen == 0 || noDataInRange) {
			...
			for (let i = mode == 2 ? 1 : 0; i < series.length; i++) {
				...
				shouldSetLegend =
					shouldSetLegend || yVal2 != yVal1 || idx2 != idx1;
				...
				if (i > 0 && s.show) {
					...
					if (cursorFocus && yVal2 != null) {
						let mouseYPos =
							scaleX.ori == 1 ? mouseLeft1 : mouseTop1;
						let dist = abs(
							focus.dist(self, i, idx2, yPos, mouseYPos)
						);
			...
			if (cursorOnePt) {
				...
				let focusChanged =
					focusedSeries == null
						? closestDist <= p
						: closestDist > p || closestSeries != focusedSeries;
			...
			if (_pub) {
				if (syncKey != null) {
					...
					syncOpts.values[0] =
						xSyncKey != null
							? posToVal(
									scaleX.ori == 0 ? mouseLeft1 : mouseTop1,
									xSyncKey
							  )
							: null;
					syncOpts.values[1] =
	...
	function mouseLeave(e, src, _l, _t, _w, _h, _i) {
		...
		if (dragging) {
			...
			if (dragH && dragV) {
				...
				snapH =
					mouseLeft1 <= snapProx ||
					mouseLeft1 >= plotWidCss - snapProx;
				snapV =
					mouseTop1 <= snapProx || mouseTop1 >= plotHgtCss - snapProx;
			}
	...
	function fire(evName, a1, a2) {
		if (deferHooks) hooksQueue.push([evName, a1, a2]);
		else {
			if (evName in hooks) {
	...
	function _init() {
		...
		shouldSetSelect =
			select.show && (select.width > 0 || select.height > 0);
		shouldSetCursor = shouldSetLegend = true;
